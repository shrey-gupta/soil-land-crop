...
│
├── Attention Layer
│   ├── Linear(hidden_dim → 1) → compute attention score per time step
│   ├── Softmax over sequence length → normalize to attention weights
...
│
└── Output: Regression or classification output depending on output_dim
'''
...
def main():
    ...
    test_years = list(range(2000, 2019)) 
...
### Comments:
## 1. Each year should have a new model
## 2. Performance report should be over a bunch of values. So predict for 2000 to 2019 maybe and then determine the performance report.
## 3. 200 epochs is enough for convergence.
## 4. Including shorter time-steps since 19 worked so well?